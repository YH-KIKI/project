...
('쑥쑥 자라요 냠냠이', 'nyam_lv2.png', 11),  -- [수정] 2 -> 11
('프로 식단러 냠냠이', 'nyam_lv3.png', 31),  -- [수정] 3 -> 31
('건강 마스터 냠냠이', 'nyam_lv4.png', 61),  -- [수정] 4 -> 61
('다이어트 신 냠냠이', 'nyam_lv5.png', 91),  -- [수정] 5 -> 91
('전설의 냠냠이', 'nyam_lv6.png', 99),      -- [수정] 6 -> 99

-- 2. 로로 시리즈 (Type 2)
('식단 병아리 로로', 'roro_lv1.png', 1),
('쑥쑥 자라요 로로', 'roro_lv2.png', 11),
('프로 식단러 로로', 'roro_lv3.png', 31),
('건강 마스터 로로', 'roro_lv4.png', 61),
('다이어트 신 로로', 'roro_lv5.png', 91),
('전설의 로로', 'roro_lv6.png', 99),

-- 3. 탄탄이 시리즈 (Type 3)
('식단 병아리 탄탄이', 'tan_lv1.png', 1),
('쑥쑥 자라요 탄탄이', 'tan_lv2.png', 11),
('프로 식단러 탄탄이', 'tan_lv3.png', 31),
('건강 마스터 탄탄이', 'tan_lv4.png', 61),
('다이어트 신 탄탄이', 'tan_lv5.png', 91),
('전설의 탄탄이', 'tan_lv6.png', 99),

-- 4. 꿈꿈이 시리즈 (Type 4)
('식단 병아리 꿈꿈이', 'kku_lv1.png', 1),
('쑥쑥 자라요 꿈꿈이', 'kku_lv2.png', 11),
('프로 식단러 꿈꿈이', 'kku_lv3.png', 31),
('건강 마스터 꿈꿈이', 'kku_lv4.png', 61),
('다이어트 신 꿈꿈이', 'kku_lv5.png', 91),
('전설의 꿈꿈이', 'kku_lv6.png', 99);
...
DELIMITER $$
CREATE PROCEDURE InsertAllLevels()
BEGIN
    DECLARE lv INT DEFAULT 1;
    DECLARE accum_exp INT DEFAULT 0; -- 누적 경험치
    DECLARE req_exp INT DEFAULT 100;  -- 다음 레벨까지 필요한 순수 경험치
    
    WHILE lv <= 99 DO
        -- 기획 구간별 레벨업 필요 경험치 적용
        IF lv >= 1 AND lv <= 10 THEN
            SET req_exp = 100;         -- 🐣 식단 병아리 구간 (필요량 100)
        ELSEIF lv >= 11 AND lv <= 30 THEN
            SET req_exp = 250;         -- 🌱 쑥쑥 자라요 구간 (필요량 250)
        ELSEIF lv >= 31 AND lv <= 60 THEN
            SET req_exp = 500;         -- 🍎 프로 식단러 구간 (필요량 500)
        ELSEIF lv >= 61 AND lv <= 90 THEN
            SET req_exp = 750;         -- 🏋️ 건강 마스터 구간 (필요량 750)
        ELSEIF lv >= 91 AND lv <= 98 THEN
            SET req_exp = 1000;        -- 👑 다이어트 신 구간 (필요량 1000)
        ELSE
            SET req_exp = 0;           -- 🌟 전설의 냠냠이 (만렙 누적 60,000 XP)
        END IF;

        IF lv = 99 THEN
            INSERT INTO highest_exp (he_level, he_exp) VALUES (99, 60000);
        ELSE
            INSERT INTO highest_exp (he_level, he_exp) VALUES (lv, accum_exp);
        END IF;
        
        SET accum_exp = accum_exp + req_exp;
        SET lv = lv + 1;
    END WHILE;
END$$
DELIMITER ;

CALL InsertAllLevels();
DROP PROCEDURE IF EXISTS InsertAllLevels;