...
DELETE FROM highest_exp WHERE he_level >= 0;

INSERT INTO highest_exp (he_level, he_exp)
WITH RECURSIVE lv_series AS (
    -- 1레벨 초기값 설정
    SELECT 1 AS lv, 0 AS total_exp
    UNION ALL
    -- 99레벨까지 반복하며 구간별 필요 경험치를 누적 합산
    SELECT 
        lv + 1,
        total_exp + CASE 
            WHEN lv + 1 <= 10 THEN 100   -- Lv 1~10: 레벨당 100 XP
            WHEN lv + 1 <= 30 THEN 250   -- Lv 11~30: 레벨당 250 XP
            WHEN lv + 1 <= 60 THEN 500   -- Lv 31~60: 레벨당 500 XP
            WHEN lv + 1 <= 90 THEN 750   -- Lv 61~90: 레벨당 750 XP
            ELSE 1000                    -- Lv 91~99: 레벨당 1000 XP
        END
    FROM lv_series
    WHERE lv < 99
)
SELECT lv, total_exp FROM lv_series;
